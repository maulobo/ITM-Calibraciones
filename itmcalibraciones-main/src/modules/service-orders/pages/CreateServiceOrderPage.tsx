...
import { X, ArrowLeft, ArrowRight, Save, Plus, User, UserPlus } from "lucide-react";
...
export const CreateServiceOrderPage = () => {
  ...
  const { data: offices, isLoading: isLoadingOffices, error: officesError } = useOfficesByClient(selectedClient);
  ...
  const { control, register, formState: { errors }, trigger, getValues, setValue, watch } = useForm<CreateServiceOrderDTO>({
    defaultValues: {
      contact: {
        name: "",
        email: "",
  ...
  const clientData = clients?.find(c => c._id === selectedClient || c.id === selectedClient);
    ...
    <Box sx={{ p: 4, maxWidth: 1200, margin: "0 auto" }}>
      ...
      <Paper sx={{ p: 4, borderRadius: 3, mb: 3 }}>
          ...
          <Grid container spacing={3}>
            ...
            <Grid size={{ xs: 12, md: 6 }}>
               <Controller
                 ...
                 render={({ field }) => (
                   <FormControl fullWidth error={!!errors.office} disabled={!selectedClient || isLoadingOffices}>
                     <InputLabel>
                     ...
                     <Select {...field} label="Oficina">
                       {!offices || offices.length === 0 ? (
                         <MenuItem disabled value="">
                           {selectedClient ? "No hay oficinas para este cliente" : "Seleccione un cliente primero"}
                         </MenuItem>
                       ) : (
                         offices.map(office => (
                     ...
                     <FormHelperText>{errors.office?.message || (selectedClient && offices?.length === 0 && "Este cliente no tiene oficinas registradas")}</FormHelperText>
                   </FormControl>
                 )}
               />
            </Grid>
          ...
          <Grid container spacing={3}>
              ...
              <Grid size={{ xs: 12 }}>
                <FormControl fullWidth>
                  ...
                  <Select
                    ...
                    label="Contacto"
                    renderValue={(value) => {
                      const contact = contactOptions.find(c => c.id === value);
                      return contact ? (
            ...
            {selectedContactId && !showAddContact && (
              <>
                <Grid size={{ xs: 12 }}>
                  <Divider sx={{ my: 1 }}>
                    <Chip label="Información del Contacto Seleccionado" size="small" />
                  </Divider>
                </Grid>
                <Grid size={{ xs: 12, md: 6 }}>
                  <TextField 
                    ...
                    fullWidth
                    size="small"
                    {...register("contact.name", { required: "Nombre requerido" })}
                ...
                <Grid size={{ xs: 12, md: 6 }}>
                  <TextField 
                    ...
                    fullWidth
                    size="small"
                    {...register("contact.email", { required: "Email requerido" })}
                ...
                <Grid size={{ xs: 12, md: 6 }}>
                  <TextField 
                    ...
                    fullWidth
                    size="small"
                    {...register("contact.phone", { required: "Teléfono requerido" })}
                    ...
                    <TableBody>
                       {items.map((item, idx) => (
                         <TableRow key={item._tempId || idx}>
                           <TableCell>
                             <Typography fontWeight="bold">{item._modelData?.name || "Modelo Desconocido"}</Typography>
                             <Typography variant="caption" color="text.secondary">
                               {item._modelData?.brand ? 
                                 (typeof item._modelData.brand === 'object' ? item._modelData.brand.name : "Marca ID") 
                                 : "-"
                               }
                             </Typography>
                           </TableCell>
                           ...
                           <TableCell align="right">
                              <IconButton color="error" size="small" onClick={() => removeItem(item._tempId)}>
                                <X size={18} />
                              </IconButton>
                           </TableCell>
                         </TableRow>
        ...
        <Box sx={{ display: activeStep === 2 ? "block" : "none" }}>
          <Box sx={{ textAlign: "center", py: 4 }}>
             ...
             <Card sx={{ maxWidth: 600, mx: "auto", mb: 3, textAlign: "left", p: 3, borderRadius: 3 }} variant="outlined">
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid size={{ xs: 6 }}>
                     ...
                     <Typography>{offices?.find(o => (o._id || o.id) === selectedOffice)?.name || "-"}</Typography>
                  </Grid>
                  <Grid size={{ xs: 6 }}>
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between" }}>
         ...
         <Button 
           ...
           sx={{ borderRadius: 3, px: 4 }}
           endIcon={activeStep === steps.length - 1 ? <Save size={18} /> : <ArrowRight size={18} />}
         >
