  ...
  async importInstruments(importInstrumentsDTO: ImportInstrumentsDTO) {
    ...
    const promises = instruments.map(async (instrument) => {
      ...
      if (!foundBrands) {
        ...
        // TODO: equipmentType ahora es requerido, necesitas obtenerlo del excel
        console.warn(
          `Cannot create model ${modelName} without equipmentType. Skipping...`,
        );
        return null; // Salta este instrumento

        /* const newModel = await this.modelService.addModel({
          name: modelName,
          brand: newBrand._id,
          equipmentType: ??? // REQUERIDO
        });
        ...
        if (models.length === 0) {
          // TODO: equipmentType ahora es requerido
          console.warn(
            `Cannot create model ${modelName} without equipmentType. Skipping...`,
          );
          return null; // Salta este instrumento
      ...
      const instrumentsTypes =
        await this.equipmentTypesService.getAllEquipmentTypes();
      ...
      if (!foundType) {
        try {
          const newInstrumentType =
            await this.equipmentTypesService.addEquipmentType({
    ...
    const promises = offices.map(async (office) => {
      try {
        ...
        if (
          ...
          ifExist.find(
            (office) =>
              office.city._id.toString() === newOffice.city.toString(),
          )
        ) {
