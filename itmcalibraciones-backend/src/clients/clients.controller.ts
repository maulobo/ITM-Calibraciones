...
export class ClientController {
  ...
  @Get("/all")
  async getAllClients(@Query() params: GetClientDTO): Promise<IClient[]> {
    return await this.clientService.getAllClients(convertToObjectId(params));
  }

  @Delete("/:id")
  async deleteClient(@Param("id") id: string): Promise<{ deleted: boolean }> {
    return await this.clientService.deleteClient(new Types.ObjectId(id));
  }
}
