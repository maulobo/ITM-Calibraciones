...
export class PdfGeneratorController {
  ...
  @Get("badget/:id")
  async generateBadgetPDF(@Res() res, @Param() param: IdParamDTO) {
    const id = convertMongoId(param.id);

    const selectBadget: IBadget = await this.badgetService.getAllBadgets({
      _id: id,
      populate: [
        "advisor",
        "office",
  ...
  async generateTechnicalInformPDF(@Res() res, @Param() param: IdParamDTO) {
    const id = convertMongoId(param.id);

    const selectInform = await this.TechnicalInformService.getInforms({
      _id: id,
      populate: [
        "equipment.instrumentType",
        "equipment.model",
        "equipment.office",
        "equipment.office.client",
      ],
    });
    const inform = selectInform[0];

    const instrument = inform.equipment as any as IEquipment;
    const date = `${("0" + inform.date.getDate()).slice(-2)}-${(
      "0" +
      (inform.date.getMonth() + 1)
    ).slice(-2)}-${inform.date.getFullYear()}`;
    ...
    const TechnicalInform: GeneratePDFTechnicalInformDTO = {
      ...
      instrumentType:
        (
          (instrument.model as any as IModel)
            .equipmentType as any as IEquipmentTypes
        )?.type || "Instrumento",
    };

    const buffer = await this.pdfGeneratorService.TechnicalInform(
      TechnicalInform,
    );
    res.set({
      // pdf
  ...
  @Get("sticker/:id")
  async generateStickerPDF(@Res() res, @Param() param: IdParamDTO) {
    const id = convertMongoId(param.id);

    const instruments = await this.equipmentService.getAllEquipments({
      _id: id,
    });
