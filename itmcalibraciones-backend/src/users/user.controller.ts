...
export class UserController {
  ...
  @Get("/")
  async getUsers(@Query() findUser: FindUserDTO): Promise<IUser[]> {
  ...
  async addUser(
    ...
    // Solo ADMIN puede crear otros ADMIN o TECHNICAL
    const isCreatingElevatedRole = createUserDto.roles.some(
      (role) => role === UserRoles.ADMIN || role === UserRoles.TECHNICAL,
    );

    if (
      isCreatingElevatedRole &&
      !currentUser.roles?.includes(UserRoles.ADMIN)
    ) {
      throwException(StatusEnum.UNAUTHORIZED);
    }

    // Los usuarios tipo USER (clientes) requieren office
    if (createUserDto.roles.includes(UserRoles.USER) && !createUserDto.office) {
  ...
  async updateUser(@Body() updateUserDTO: UpdateUserDTO): Promise<IUser> {
    ...
    updateUserDTO.password &&
      ...
      this.userService.notifyUpdateUser({
  ...
  @Auth()
  @Get("/me")
  async getMyProfile(@User() user: JwtPayload): Promise<any> {
  ...
  @Auth(UserRoles.ADMIN, UserRoles.TECHNICAL)
  @Get("/admins")
  async getAdminTechUsers(@User() user: JwtPayload): Promise<any> {
  ...
  async updateMyProfile(
    ...
    updateUserProfileDTO.password &&
      updateUserProfileDTO.password.length > 0 &&
      this.userService.notifyUpdateUser({
        updatedUser: updatedUser,
        notifyPassword: true,
      });
