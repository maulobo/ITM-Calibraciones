...
export class CertificateService {
    ...
    async addCertificateWithWaterMark(addCertificateDTO: AddCertificateDTO, file: Express.Multer.File) { 
        ...
        const equipmentData: IEquipment[] = await this.equipmentService.getAllEquipments(params)
        const socialReason = (((equipmentData[0].office as any as IOffice).client) as any as IClient).socialReason
        ...
        try {
            ...
            const {watermarkedFilePath}  = await this.imageUploadService.addWatermark(localFilePath, fileName);
            const destinationS3FilePath = `certificate/${socialReason}/${fileName}`

            const uploadeLocation = await this.imageUploadService.uploadToS3(watermarkedFilePath, destinationS3FilePath);
            ...
            const calibrationExpirationDate = new Date(addCertificateDTO.calibrationExpirationDate)
            ...
            return this.commandBus.execute(new AddCertificateCommand(addCertificateDTO));
        } catch (error) {
            // Handle error
    ...
    async addCertificate(addCertificateDTO: AddCertificateDTO, request) { 
        ...
        equipmentData[0].calibrationDate = new Date(addCertificateDTO.calibrationDate)
        equipmentData[0].calibrationExpirationDate = new Date(addCertificateDTO.calibrationExpirationDate)
        
        await equipmentData[0].save()
        return this.commandBus.execute(new AddCertificateCommand(addCertificateDTO));
    }

    async getCertificate(getCertificateDTO:GetCertificateDTO): Promise<ICertificate[]> { 
        return this.queryBus.execute(new FindCertificateQuery(getCertificateDTO));
    }

    async deleteCertificate(deleteCertificate:DeleteCertificateDTO): Promise<ICertificate> {
        ...
        if(deleteCertificate.equipment && deleteCertificate.certificate){
            if(
                ...
                && (deleteCertificate.equipment as IEquipment).certificate === deleteCertificate.certificate){
                    return throwException(StatusEnum.ERROR_DELETE_CERTIFICATE_UNDER_USING);

            }
        }
        deleteCertificate.deleted = true 
        return this.commandBus.execute(new DeleteCertificateCommand(deleteCertificate));
    }

    async certificateUserAccess(user: JwtPayload, certificate:ICertificate):Promise<boolean>{
        const userOffice = user.office
        return (certificate.equipment as any as IEquipment).office?._id.toString() === userOffice
    }

    async cleanCertificateUserAccess(user: JwtPayload, certificates:ICertificate[]):Promise<ICertificate[]>{
